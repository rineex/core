...
- Replace `Result` class with a frozen discriminated union
  (`kind: 'ok' | 'err'`), ([#62](https://github.com/rineex/core/pull/62)) add
  combinators (`match`, `flatMap`, `map`, `mapError`), and introduce
  `UseCaseError` seam.
  ...
  **Breaking changes:**
  ...
  - `getValue()` / `getError()` → `result.value` / `result.error` after
    narrowing
